...
jiraConnect.prototype.fetchProjectSprintsbyrapidview = function fetchProjectSprintsbyrapidview (config, callback) {
        console.log("Connection Started for fetching project 's all sprints");
        var cb = callback || null,
           endpoint = config.url,
            options = {
                url: endpoint,
...
jiraConnect.prototype.fetchProjectRapidviews = function fetchProjectRapidviews (config, callback) {
        console.log("Connection Started for fetching rapidview 's sprintid "+ config.url);
        var cb = callback || null,
            endpoint = config.url,
               // .replace(/{sprintid}/g, config.sprintid),
            options = {
